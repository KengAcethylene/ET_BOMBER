...
public class BombArea {
	...
	public void setAfterDestroy() {
		if (afterDestroy[0] >= 0)
			isCanShowTop[afterDestroy[0]] = true;
		if (afterDestroy[1] >= 0)
			isCanShowBot[afterDestroy[1]] = true;
		if (afterDestroy[2] >= 0)
			isCanShowLeft[afterDestroy[2]] = true;
		if (afterDestroy[3] >= 0)
			isCanShowRight[afterDestroy[3]] = true;
	}
	...
	public boolean[] setIsCanShowBySite(Site site) {
		...
		for (int j = 0; j < getRange(); j++) {
			...
			if (!isCanShow[j]) {
				if (site == Site.TOP && bomb.getGameController().getObjectInGame(xPos,
						Math.max(yPos - j - 1, 0)) != ObjectInGame.WALL)
					afterDestroy[0] = j;
				else if (site == Site.BOT && bomb.getGameController().getObjectInGame(xPos,
						Math.min(yPos + j + 1, 14)) != ObjectInGame.WALL)
					afterDestroy[1] = j;
				else if (site == Site.LEFT && bomb.getGameController().getObjectInGame(Math.max(xPos - j - 1, 0),
						yPos) != ObjectInGame.WALL)
					afterDestroy[2] = j;
				else if (site == Site.RIGHT && bomb.getGameController().getObjectInGame(Math.min(xPos + j + 1, 14),
						yPos) != ObjectInGame.WALL)
					afterDestroy[3] = j;
				break;
			}
